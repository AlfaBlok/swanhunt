...
def get_peaks_troughs(log_data, distance_param, prominence_param, divisions):
    # Find peaks and troughs
    # st.dataframe(log_data)
    # we manually set distance_parameter as type timedelta
    distance_param = 30.5
    print('Barlam')
    print('Distance:', distance_param)
    # we print the type of distance
    print('Type of distance:', type(distance_param))
    print(log_data.head())
    # log_data.set_index('date', inplace=True)
    peaks = find_peaks(log_data, distance=(distance_param), prominence=prominence_param)[0]
    troughs = find_peaks(-log_data, distance=(distance_param), prominence=prominence_param)[0]
    print('Peaks:', peaks)
    ...
    for i,peak in enumerate(peaks):
        ...
        immediate_prices = historical_data['close_price'].iloc[peak+1:peak+int(distance)]
...
def animate(i, full_data, distance_param, prominence, divisions,min_x, max_x,min_y, max_y, symbol, min_date_model, min_year_ani,axs, alpha, transparency, shaded = False, style = 'Simple'):
    ...
    print('Hello')
    print('Type of distance_param: ', type(distance_param))
    print('Distance: ', distance_param)
    print('temp_data next: ',temp_data)
    # we set 'date' as index
    # temp_data = temp_data.set_index('date')
    clean_peaks, clean_troughs = get_peaks_troughs(temp_data['close_price'], distance_param, prominence, divisions)
...
final_year_model_prediction = 2030 #sidebar.slider('Final Prediction', 2025, 2050, 2030,5)
...
distance = 200.0 # sidebar.slider('Distance', 10, 1000, 100)
default_prominence = (np.ceil(log10_hist.max()) - np.floor(log10_hist.min()))/20
# st.text(f'Default prominence: {default_prominence}')

prominence = 0.2 # sidebar.slider('Prominence', 0.1, 1.0, 0.2,0.1)
divisions = 4 # sidebar.slider('Divisions', 2, 10, 4)

print('Min date loaded: ', full_data['date'].min())
print('Max date loaded: ', full_data['date'].max())


clean_peaks, clean_troughs = get_peaks_troughs(log_data, distance, prominence, divisions)

retained_peak_prices = np.array(log_data[clean_peaks])
retained_trough_prices = np.array(log_data[clean_troughs])

min_x =  full_data['date'].min()   # sidebar.date_input('Min Date', value=full_data['date'].min())
...
min_y = 1e-4 # 10**sidebar.number_input('Min Y (1e)', value=default_min_y)
max_y = 1e6 # 10**sidebar.number_input('Max Y (1e exponent)', value=default_max_y)
...
style = 'Radar' # sidebar.radio('Choose chart style', ['Simple','Radar'])
alpha = 0.5 #sidebar.slider('Alpha', 0.0, 1.0, 0.5)
transparency = True # sidebar.checkbox('Trailing')
shaded = True #sidebar.checkbox('Shaded Area')
months_step = 48 #sidebar.slider('Months step', 1, 60, 12)
...
min_date_model = full_data['date'].min() #sidebar.date_input('Min Date Model', value=)
min_year_ani = 2020 #sidebar.slider('Min Year Animation', 1995, 2025, 2020)
max_year_ani = 2025 #sidebar.slider('Max Year Animation', 2025, 2050, 2030)


fig, axs = plt.subplots(1 if style == 'Simple' else 2, 3, figsize=(14, 7))
f_args = (full_data, distance, prominence, divisions, min_x, max_x, min_y, max_y, symbol, min_date_model, min_year_ani,axs, alpha, transparency, shaded, style)
